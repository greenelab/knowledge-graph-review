...
Entities mentioned in these databases are represented as nodes while the type of database indicates the edge both entities share.
For example, a graph using DISEASES [@doi:10.1016/j.ymeth.2014.11.020] as a resource would contain nodes that represent genes and diseases and edges between the nodes representing an association.
This example contains a single type of relationship; however, there are databases that can infer multiple relationships.
