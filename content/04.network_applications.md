...
Typically, input for this class of methods is an adjacency matrix ($A$).
This matrix is a square matrix where rows and columns represent nodes and each entry is a binary represention of the presence of an edge between two nodes.
In this form all edges are treated as equal, which means information such as edge type is lost. 
Once $A$ has been constructed, algorithms such as SVD [@doi:10.1007/BF02288367] decomposes $A$ into the following three forms: a square matrix $Σ$ and a set of two small rectangular matrices $U$ and $V^{T}$.
...
In practice $U$ is usually used to represent nodes in a knowledge graph and is used as input for machine learning classifiers to perform link prediction or node classification [@doi:10.1093/bioinformatics/btz718]; however,$V^{T}$ can also be used [@doi:10.1007/BF02288367; @raw:Jiezhon2018].
