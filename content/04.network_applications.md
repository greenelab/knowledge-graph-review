...
One of the pioneering approaches within this paradigm is TransE [@raw:bordestranse].
TransE treats every node-edge pair as a triplet with head nodes represented as $\textbf{h}$, edges represented as $\textbf{r}$, and tail nodes represented as $\textbf{t}$.
