...
Starting with a knowledge graph one can generate embeddings using Matrix Factorization (a), Translational Models (b) and even Deep Learning (c).
The output of this pipeline is an embedding space where common node types are grouped together.
...
Notable approaches in this category are Isomap [@doi:10.1126/science.290.5500.2319], Laplacian eigenmaps [@doi:10.1162/089976603321780317] and Pincipal Component Analysis (PCA) [@doi:10/bm8dnf] or Singular Vector Decomposition (SVD) [@doi:10.1007/BF02288367].
...
SVD [@doi:10.1007/BF02288367] is an approach that can project knowledge graphs into a low dimenisional space.
The input for this technique is an adjacency matrix ($A$), which is a square matrix where rows and columns represent nodes in a knowledge graph and each entry represents the presence of an edge between two nodes. 
The adjacency matrix is decomposed into three parts: a set of two small rectanguar matricies $U$ and $V^{T}$ and a square matrix $Σ$.
This values within $Σ$ are called singular values, which are similar to eigenvalues [@doi:10.1007/BF02288367].
Each row in $U$ represents nodes embedded into a low dimensional space while each column in $V^{T}$ represents nodes mapped onto a low dimensional space [@doi:10.1007/BF02288367; @doi:10/bm8dnf].
...
This technique first converts an adjacency matrix ($A$) into a laplacian matrix ($L$) by subtracting the adjacency matrix from the graph's degree matrix ($D$): a diagonal matrix where each entry represents the number of edges connected to a node.
After calculating the laplacian matrix, this algorithm then uses linear algebra to calculate eigenvalues and eigenvectors from the laplacian matrix ($Lx = \lambda Dx$).
The generated eigenvectors are the knowledge graph's nodes projected into a low dimensional space [@doi:10.1162/089976603321780317].
...
Typically eigenmaps work well when knowledge graphs have sparse connections between nodes, but struggle with dense networks [@arxiv:1906.05017; @doi:10.1371/journal.pcbi.1005621; @arxiv:1905.09763].
A future direction is to adapt these methods to accommodate different knowledge graph structures.

Matrix factorization is a powerful technique that uses a matrices such as  an adjacency matrix as input.
Common approaches involve using either SVD or Laplacian eigen maps to perform embeddings.
Despite reported success, the dependence on matrices like an a adjacency matrix creates an issue of scalability.
As knowledge graphs asymptotically increases in the number of nodes and edges, these approaches would be hard to utilize.
Furthermore, these methods treat all edge types the same, but a possible extension would be to incorporate node and edge type as sources of input.
