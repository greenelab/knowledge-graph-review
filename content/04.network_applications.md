...
In the context of knowledge graphs the graph representation is already defined, which means one can move on towards the next step.
The second step of this technique is to obtain both an adjacency matrix ($A$) and a degree matrix ($D$) from graph representation.
...
Despite reported success, the dependence on matrices creates an issue of scalability as matrices of large networks would take too much memory for a regular computer to operate well.
