...
This bottleneck highlights a critical need for future approaches to scale fast enough to comprete with the increasing publication rate.
...
The first step of these methods is to use an automated system to initially extract sentneces from text.
This process removes irrlevant sentences, which dramatically decreases the amount of text curators will have to sift through.
