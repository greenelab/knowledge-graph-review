...
Usually, knowledge graphs are constructed using pre-existing databases and these databases are constructed by domain experts using approaches ranging from manual curation to automated techniques, such as text mining systems.
Manual curation is an extensive process that involves domain experts reading papers and detecting sentences that assert a relationship.
Instead of using domain experts to read papers automated approaches rely on machine learning or natural language processing techniques to rapidly detect sentences of interest.
...
This database has reached close to 35M entries in 2016 [@doi:10.1093/nar/gkw1121] and grew to a total of 45M entries in 2019 [@doi:10.1093/nar/gky1015].
...
This process removes irrelevant sentences, which dramatically decreases the amount of text curators will have to sift through.
Following the pre-filtering step, curators will then need to approve or reject the remaining sentences.
It was discovered that this approach saved curators an average of 2-2.8 hours compared to manual efforts [@doi:10.1016/j.jbi.2010.11.001; @doi:10.1136/amiajnl-2013-001837]. 
Despite automated systems excelling in identifying sentences for commonly occurring relationships, they tend to miss out on lessor known relationships [@doi:10.1016/j.jbi.2010.11.001].
Plus, these systems have a hard time parsing ambiguous sentences that naturally occur in text, which makes correcting them a challenging task [@doi:10.1016/j.jbi.2010.11.001].
...
Rule based-extraction consist of identifying essential keywords and grammatical patterns to detect relationships of interest. 
...
In this section we focus on approaches that use mainly rule based extraction to detect sentences that allude to a relationship.
...
As a consequence the simplifier missed out on detecting relevant sentences [@doi:10.1093/database/bav020].
