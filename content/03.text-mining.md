...
A branch of unsupervised methods exploit the fact that two entities can appear together in text. 
This kind of event is called co-occurrence and studies that use this phenomon can be found in table {@tbl:unsupervised-methods-text-mining}.
...
Biomedical relationships consists of multiple types that are distinct from each other.
Clustering is an approach that can simultaneously capture the heterogenous nature of these relationships.
