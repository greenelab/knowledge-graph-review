...
Constructing a knowledge graph involves a two step process.
The first step is to define a representation for relevant entities.
Usually, this involves using multiple ontologies to represent entities [@doi:10.1007/3-540-45034-3_2].
The second step is to extract information from structured resources, e.g. databases, to establish relationships between the various entites.
Generally these databases are populated via some form of manual curation.
This process involves the use of domain experts to read papers and identify relevant sentences that assert a relationship.
Besides plain curation, other approaches have moved towards automated systems to detect sentences.
These systems combine machine learning with natural lanauge processing techniques to perform relationship extraction.
We categorize these systems into the following groups:  rule-based extraction, unsupervised machine learning, and supervised machine learning.
In this section we discuss the pros and cons towards each of the described approaches to populating databases.
